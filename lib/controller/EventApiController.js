 ...
 *
 * @Firewall(realm="ClientProfiler_EventApi",
 *           shared=false,
 *           stateless=false,
 *           roles="ROLE_CLIENT_PROFILER_API",
 *           provider="ClientProfiler_EventApi",
 *           authenticator="ClientProfiler_EventApi")
 */
        ...
        return new Promise((resolve, reject) => {
            ...
            for (const bufferPayload of json.eventBuffer) {
                ...
                bufferPayload.buffer.sort((a,b) => {
                    a = a.payload.eventPayload.timeStamp || a.payload.eventPayload.metadata.time;
                    b = b.payload.eventPayload.timeStamp || b.payload.eventPayload.metadata.time;
                    return a - b;
                });
                ...
                for (const buffer of bufferPayload.buffer) {
                    ...
                    if (metadata.isHistoryEvent ||
                        ...
                        page = {
                            bufferEventIdx,
                            url: metadata.url,
                            startTime: payload.timeStamp || metadata.time
                        };
                        pages.push(page);
                        pageIdx += 1;
                        payload.isNewPage = true;
                    }
                    payload.bufferPageIdx = pageIdx;
                }
            }
