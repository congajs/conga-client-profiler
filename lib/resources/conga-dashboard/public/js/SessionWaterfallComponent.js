...
const styleEventMarkers = eventData => {
    for (const [index, event] of eventData.events.entries()) {
        ...
        if (payload.isNewPage) {
            ...
            left = elWidth * ((eventPayload.timeStamp - pages[0].startTime) / eventData.duration);
        } else {
            ...
            left = elWidth * ((eventPayload.timeStamp - page.startTime) / page.duration);
            ...
            <table class="waterfall">
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td class="timeline">
                            ...
                            <span class="event-info is-size-7">
                                ...
                                <span v-else-if="event.payload.type === 'xhr' && event.payload.eventPayload.event.type === 'load'">
                                    ...
                                    <span v-if="event.payload.data.responseTextBytes >= 150000">
                                        ~{{ (event.payload.data.responseTextBytes / 1000000).toFixed(1) }} MB 
                                    </span>
                                    <span v-if="event.payload.data.responseTextBytes >= 500">
                        ...
                        <td class="date">
                            {{ new Date(event.payload.eventPayload.timeStamp) | moment('H:mm:ss') }}.{{ new Date(event.payload.eventPayload.timeStamp) | moment('SSS') }}
                        </td>
                    </tr>
                </tbody>
            </table>
